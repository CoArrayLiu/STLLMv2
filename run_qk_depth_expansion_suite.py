#!/usr/bin/env python3
"""Run the six 7/8/9-layer QK expansion experiments sequentially.

By default, each of the six architecture designs is trained independently on
all four Taxi/Bike datasets before the runner advances to the next design.
All child processes use one GPU sequentially and never overwrite prior runs.
"""
...
from train_qk_depth_expansion import (
    EXPANSION_VARIANTS,
...
DEFAULT_VARIANTS = tuple(EXPANSION_VARIANTS)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output_root",
        type=Path,
        default=Path("logs/qk_depth_expansion_seed6666"),
    )
...
def child_command(
    ...
    command = [
        sys.executable,
        "-u",
        str(Path(__file__).with_name("train_qk_depth_expansion.py")),
        "--variant",
        variant,
        "--data",
        dataset,
...
def result_row(summary: dict) -> dict:
    ...
    return {
        ...
        "added_layers": -int(spec["removed_layers"]),
...
def main() -> None:
    ...
    print(
        f"Six-design QK depth-expansion suite: {len(args.variants)} variants × "
